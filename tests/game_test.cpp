...
public:
    ...
    void update(float delta_time)
    {
        ...
        else if (_state == Alive)
        {
            ...
            float dist = distance(target, position);
            ...
            _velocity = glm::normalize(target - position) * _speed;
...
[[noreturn]] int main(int argc, char **argv)
{
    ...
    auto [box_indices, box_vertices] = geometry_generators::generate_box_geometry_data(70.0f, 25.0f, 75.0f, 375, 375, 375);
    auto box_geometry = std::make_shared<ES2Geometry>(box_indices, box_vertices);
    auto box_material = std::make_shared<ES2PhongMaterial>();
    box_material->set_face_culling_enabled(true);
    auto [image1_data, image1_width, image1_height, image1_channels] = file_utilities::read_image_file("data/images/room.png");
    auto [image1_normals_data, image1_normals_width, image1_normals_height, image1_normals_channels] = file_utilities::read_image_file("data/images/room_normals.png");
    ...
    room_ground->set_position(glm::vec3(0.0f, -5.5f, 0.0f));
    room_ground->set_rotation(glm::vec3(-M_PI / 2.0f, 0.0f, 0.0f));

    // Monsters

    float enemy1_size = 7;
    float enemy1_speed = 30;
    int enemy1_sprite_frames = 7;
    int enemy1_dying_first_sprite_frame = 4;

    float enemy2_size = 7;
    float enemy2_speed = 30;
    int enemy2_sprite_frames = 5;
    int enemy2_dying_first_sprite_frame = 4;

    std::tuple enemy1_sprite_data = std::make_tuple("data/images/demon.png", enemy1_sprite_frames, enemy1_dying_first_sprite_frame);

    std::tuple enemy2_sprite_data = std::make_tuple("data/images/rock.png", enemy2_sprite_frames, enemy2_dying_first_sprite_frame);

    glm::vec3 enemy1_position{-10.0f, -3.0f, 5.0f};
    auto enemy1 = std::make_shared<Enemy>(enemy1_position, enemy1_size, enemy1_speed, enemy1_sprite_data);

    glm::vec3 enemy2_position{10.0f, -3.0f, 0.0f};
    auto enemy2 = std::make_shared<Enemy>(enemy2_position, enemy2_size, enemy2_speed, enemy2_sprite_data);
    ...
    auto [lamp_indices, lamp_vertices] = geometry_generators::generate_sphere_geometry_data(0.02f, 20, 20);
    ...
    std::vector<std::shared_ptr<Object>> objects{box, enemy1->get_mesh(), enemy2->get_mesh(), gun->get_mesh()};
    ...
    point_light1->set_constant_attenuation(1500.0f);
    point_light1->set_linear_attenuation(5.2f);
    point_light1->set_quadratic_attenuation(0.5f);
    point_light1->set_two_sided(true);
    point_light1->set_position(glm::vec3(-25.0f, -1.0f, 25.0f));
    ...
    point_light2->set_constant_attenuation(1500.0f);
    point_light2->set_linear_attenuation(5.2f);
    point_light2->set_quadratic_attenuation(0.5f);
    point_light2->set_two_sided(true);
    point_light2->set_position(glm::vec3(25.0f, -1.0f, 25.0f));
    ...
    point_light3->set_constant_attenuation(1500.0f);
    point_light3->set_linear_attenuation(5.2f);
    point_light3->set_quadratic_attenuation(0.5f);
    point_light3->set_two_sided(true);
    point_light3->set_position(glm::vec3(25.0f, -1.0f, -25.0f));
    ...
    point_light4->set_constant_attenuation(1500.0f);
    point_light4->set_linear_attenuation(5.2f);
    point_light4->set_quadratic_attenuation(0.5f);
    point_light4->set_two_sided(true);
    point_light4->set_position(glm::vec3(-25.0f, -1.0f, -25.0f));
    ...
    camera->set_position(glm::vec3(0.0f, 0.0f, 35.0f));
    ...
    // Rendering
    //     glm::vec3 player = camera->get_position();
    //     glm::vec3 en = enemy->get_mesh()->get_position();

    //     float dist = distance(player, en);

    //     if (dist < 5)
    //     {
    //         // // exit(0);

    //    //  std::vector<std::shared_ptr<Object>> objects{room_ground};
    ...
    for (;;)
    {
        ...
        for (auto &enemy : enemies)
        {
            ...
            //     objects.push_back(room_ground);

            //     scene = std::make_shared<Scene>(objects);
